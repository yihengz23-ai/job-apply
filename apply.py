...
REPORT_PATH = "/path/to/your/report.pdf"
...
7. 关注行业（focus_industry）— 该岗位主要 cover 的行业/赛道，如"AI"、"半导体"、"医疗健康"、"消费"、"新能源"、"先进制造"、"TMT"、"硬科技"等；从 JD 中提取，提取不到则填空字符串
...
姓名：YOUR_NAME
英文名：YOUR_ENGLISH_NAME（仅英文邮件或英文 JD 可考虑使用）
邮箱：YOUR_EMAIL
电话：YOUR_PHONE

教育背景：
- 硕士：YOUR_UNIVERSITY，YOUR_SCHOOL，YOUR_PROGRAM（YOUR_PROGRAM），YOUR_GRAD_DATE 毕业
- 本科：YOUR_UNDERGRAD_UNIVERSITY，YOUR_MAJOR

邮件中教育背景的写法（非常重要）：
- 默认只写"YOUR_UNIVERSITY硕士在读"，不写学院名、不写专业名、不写 YOUR_PROGRAM
...
- 必须严格按 JD 要求生成
- 不能多一个字，也不能少一个字
- 不能擅自添加学校、项目经历、YOUR_PROGRAM、YOUR_COMPANY_1等额外信息
...
- JD 要求：【PE】姓名-年级-学校-一周几天-实习期限
  => 【PE】YOUR_NAME-硕士在读-YOUR_UNIVERSITY-一周5天-可立即到岗-YOUR_END_DATE
...
- 不要在标题中加YOUR_PROGRAM、专业名称，除非JD明确要求写专业
...
- 严格按 JD 要求生成
- 例如：学校-专业名称-毕业时间-姓名
  => YOUR_UNIVERSITY-YOUR_MAJOR-YOUR_GRAD_DATE-YOUR_NAME.pdf

2. 如果 JD 没规定：
- 默认：YOUR_NAME-YOUR_UNIVERSITY-简历.pdf
- 不要加YOUR_PROGRAM、不要加专业，除非JD明确要求写专业
...
信息精简规则（非常重要）：
- 正文中只写"YOUR_UNIVERSITY硕士在读"，不要写"YOUR_SCHOOL"，不要写"YOUR_PROGRAM"，不要写 YOUR_PROGRAM
...
- 候选人最晚可实习到 YOUR_END_DATE，从当前时间算可实习的最大月数，取 JD 范围内的最大值
- 例如：JD 写"3-6个月" → 写"可实习6个月"
- 例如：JD 写"至少3个月" → 写"可实习6个月"
- 不要写超过实际可实习时间（YOUR_END_DATE 之前）
...

{
  ...
  "email_body": "",
  "company_name": "公司名称",
  "company_type": "公司性质（如：外资精品投行、本土PE、美元VC、国资PE、四大咨询等）",
  "job_title": "岗位名称",
  "job_location": "工作地点",
...
def send_gmail(service, to_email, subject, body,
               ...
               attach_report, report_filename):
    """直接发送 Gmail 邮件（含附件），返回 message_id"""
    if not to_email:
...
def update_record_status(record_id, new_status):
    ...
    for r in records:
        if r.get("id") == record_id:
            r["status"] = new_status
            r["status_updated_at"] = datetime.now().strftime("%Y-%m-%d %H:%M")
            save_records(records)
            return True
    return False
